...
UniValue GetNextMasternodeForPayment(int heightShift)
{
    ...
    obj.pushKV("payee",         payeeAddr.IsValid() ? payeeAddr.ToString() : "UNKNOWN");
    return obj;
}
...
UniValue masternodelist(const JSONRPCRequest& request)
{
    ...
    mnList.ForEachMN(false, [&](const CDeterministicMNCPtr& dmn) {
        ...
        if (strMode == "addr") {
            ...
            if (strFilter !="" && strOutpoint.find(strFilter) == std::string::npos) return;
            obj.pushKV(strOutpoint, CBitcoinAddress(dmn->pdmnState->keyIDOwner).ToString()));
            ...
            if (strFilter !="" && strOutpoint.find(strFilter) == std::string::npos) return;
            obj.pushKV(strOutpoint, CBitcoinAddress(dmn->pdmnState->keyIDVoting).ToString()));
        }
    });

...
void masternode_list_help()
{
    throw std::runtime_error(
            "evomasternode list ( \"mode\" \"filter\" )\n"
            "Get a list of evo masternodes in different modes. This call is identical to evomasternodelist call.\n"
            "\nArguments:\n"
...
{
    throw std::runtime_error(
            "evomasternode count (\"mode\")\n"
            "  Get information about number of evo masternodes. Mode\n"
...
UniValue GetNextMasternodeForPayment(int heightShift)
{
    ...
    CBZXAddress payeeAddr;
    if (ExtractDestination(payeeScript, payeeDest)) {
        payeeAddr = CBZXAddress(payeeDest);
    }
...
{
    throw std::runtime_error(
            "evomasternode winners ( count \"filter\" )\n"
            "Print list of evo masternode winners\n"
            "\nArguments:\n"
...
{
    throw std::runtime_error(
        "evomasternode \"command\"...\n"
        "Set of commands to execute evo masternode related actions\n"
        "\nArguments:\n"
        ...
        "  list         - Print list of all known masternodes (see evomasternodelist for more info)\n"
    ...
    mnList.ForEachMN(false, [&](const CDeterministicMNCPtr& dmn) {
        ...
        if (GetUTXOCoin(dmn->collateralOutpoint, coin)) {
            ...
            if (ExtractDestination(coin.out.scriptPubKey, collateralDest)) {
                collateralAddressStr = CBZXAddress(collateralDest).ToString();
            }
        }
        ...
        if (ExtractDestination(payeeScript, payeeDest)) {
            payeeStr = CBZXAddress(payeeDest).ToString();
        }
        ...
        if (strMode == "addr") {
            ...
            streamInfo <<  dmn->proTxHash.ToString() << " " <<
                           ...
                           dmn->pdmnState->nLastPaidHeight << " " <<
                           CBZXAddress(dmn->pdmnState->keyIDOwner).ToString() << " " <<
                           CBZXAddress(dmn->pdmnState->keyIDVoting).ToString() << " " <<
            ...
            objMN.push_back(Pair("owneraddress", CBZXAddress(dmn->pdmnState->keyIDOwner).ToString()));
            objMN.push_back(Pair("votingaddress", CBZXAddress(dmn->pdmnState->keyIDVoting).ToString()));
            ...
            if (strFilter !="" && strOutpoint.find(strFilter) == std::string::npos) return;
            obj.push_back(Pair(strOutpoint, CBZXAddress(dmn->pdmnState->keyIDOwner).ToString()));
            ...
            if (strFilter !="" && strOutpoint.find(strFilter) == std::string::npos) return;
            obj.push_back(Pair(strOutpoint, CBZXAddress(dmn->pdmnState->keyIDVoting).ToString()));
        }
    });
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    { "BZX",               "masternode",                 &masternode,             true,  {} },

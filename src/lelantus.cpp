...
void GenerateMintSchnorrProof(const lelantus::PrivateCoin& coin, CDataStream&  serializedSchnorrProof)
{
    ...
    // after nLelantusStartBlock block start to pass whole data to transcript
    ...
    // commit (G^s*H1^v*H2^r), comm (G^s*H2^r), and H1^v are used in challenge generation if nLelantusStartBlock is passed
...
bool CheckLelantusJoinSplitTransaction(
    ...
    if (jSplitVersion < LELANTUS_TX_VERSION_4 || (!isVerifyDB && jSplitVersion != LELANTUS_TX_TPAYLOAD)) {
        return state.DoS(100,
                         false,
                         NSEQUENCE_INCORRECT,
    ...
    for (auto& idAndHash : joinsplit->getIdAndBlockHashes()) {
        ...
        if (joinsplit->isSigmaToLelantus() && sigma::IntegerToDenomination(intDenom, denomination)) {
            ...
            // take the hash from last block of anonymity set, it is used at challenge generation if nLelantusStartBlock is passed
        ...
        if (id) {
            ...
            if (block->lelantusMintedPubCoins.count(id) > 0) {
                for (const auto &coin : block->lelantusMintedPubCoins[id]) {
                    LOCK(cs_main);
                    // skip mints from blacklist if nLelantusStartBlock is passed
...
void CLelantusState::GetAnonymitySet(
    ...
    const auto &params = ::Params().GetConsensus();
    LOCK(cs_main);
    int maxHeight = fStartLelantusBlacklist ? (chainActive.Height() - (ZC_MINT_CONFIRMATIONS - 1)) : (params.nLelantusStartBlock - 1);
